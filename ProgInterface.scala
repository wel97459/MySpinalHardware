...
class BaudRateGen(Baud: BigInt) extends Component {
  ...
  val counter = CounterFreeRun(
    (ClockDomain.current.frequency.getValue / Baud.toInt / 8).toInt
  )
...
class ProgrammingInterface(Baud: BigInt) extends Component {
  val io = new Bundle {
    ...
    val Floppy = new Bundle {
      val Track = in Bits (8 bit)
      val Sector = in Bits (8 bit)
      val Side = in Bool ()
      val SeekReq = in Bool ()
      val ReadReq = in Bool ()
      val WriteReq = in Bool ()
      val DataOut = slave Stream (Bits(8 bit))
      val DataIn = master Stream (Bits(8 bit))
    }
...
//Floopy disk interface

  val FDC_in = Stream(Bits(8 bits))
  val FDC_in_Fifo = StreamFifo(
    dataType = Bits(8 bits),
    depth = 128
  )
  FDC_in_Fifo.io.push << FDC_in
  FDC_in_Fifo.io.pop >> io.Floppy.DataIn

  FDC_in.valid := False
  FDC_in.payload := workingReg(7 downto 0)

  val FDC_out = Stream(Bits(8 bits))
  val FDC_out_Fifo = StreamFifo(
    dataType = Bits(8 bits),
    depth = 128
  )
  FDC_out_Fifo.io.push << io.Floppy.DataOut
  FDC_out_Fifo.io.pop >> FDC_out
  FDC_out.ready := False
  ...
  val InterfaceFMS = new StateMachine {
    ...
    val FloppySeek: State = new State {
      whenIsActive {
        when(amount === 0) {
          ...
          uart_tx.io.buffer_write := True
          when(amount === 4) {
            uart_tx.io.data_in := 'S'
          } elsewhen (amount === 3) {
            uart_tx.io.data_in := io.Floppy.Track
          } elsewhen (amount === 2) {
            uart_tx.io.data_in := io.Floppy.Sector
          } elsewhen (amount === 1) {
            uart_tx.io.data_in := Cat(B"0000000", io.Floppy.Side.asBits)
          }
          amount.decrement()
        }
      }
    ...
    val FloppyRead: State = new State {
      whenIsActive {
        when(uart_rx.io.buffer_data_present || amount === 0) {
          when(amount === 0) {
            goto(Waiting)
...
object ProgrammingInterfaceConfig
...
object ProgrammingInterfaceGen {
  def main(args: Array[String]) {
    ProgrammingInterfaceConfig
      .generateVerilog(new ProgrammingInterface(9600))
      .printPruned
  }
}
      ...
      .doSim { dut =>
        ...
        loop.breakable {
          while (true) {
            if (t > 80) {
              if (
                dut.uart_rx.io.buffer_full.toBoolean == false && d < str
                  .length()
              ) {
