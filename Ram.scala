...
class RamInit(File: String, AddrDepth: Int = 8, DataDepth: Int = 8) extends Component {
    val io = new Bundle {
        val ena     = in  Bool()
    ...
    def readBin(file : String): Seq[Bits] = Files.readAllBytes(Paths.get(file)).map(b => B(b.toInt & 0xFFFF))
    def MemRom(initialContent: Seq[Bits]) = new Mem(Bits(DataDepth bit), initialContent.length) init(initialContent)
...
class Ram(AddrDepth: Int = 8, DataDepth: Int = 8) extends Component {
    ...
    def MemRom(Depth: Int) = new Mem(Bits(DataDepth bit), max_bits(Depth))
